---
title: "Pron√≥stico de valor de acciones"
output:
...
```{r}
# invocando librer√≠as a utilizar
library(quantmod)
library(lubridate)
library(dplyr)
library(tidyr)
...
```{r}
hoy <- today()
acciones <- getSymbols("^GSPC", from = "2015-01-01", to = hoy, src = "yahoo", auto.assign =  F)[,6]
```

Hemos cargado la tabla `acciones` con la informaci√≥n del cierre de las acciones.
...
Ya tenemos un DataFrame que tiene dos columnas, una con los precios de las acciones del √≠ndice `S&P 500` y otra con las fechas.
...
Convertimos la columna `Fecha` a formato `Date`. Ahora vamos a crear un rango de fechas adicionales , que representan los dÌas que queremos pronosticar.
...
Vamos a usar un modelo de `Machine Learning` para predecir el costo de las acciones de `S&P 500` a futuro. Lo que vamos a intentar hacer es explicar la variable `Precio` a partir de tres variables independientes que ser·n `AÒo`, `Mes` y `DÌa`. Usaremos un modelo de `bosque aleatorio` que usa `¡rboles de decisiÛn`.

Tenemos que tener claro que hay comportamientos en la variable `Precio` que no se pueden predecir solamente con estas tre variables. El comportamiento a nivel local, en dÌas cercanos, depender· de otros factores. Por ejemplo, tenemos una caÌda abrusca en enero de 2020 debido a la pandemia y tenemos otra caÌda abrupta en enero de 2022 cuando comenzaron los aires de guerra en Ucrania.

Otras variables que pueden explicar el `Precio` son:

-   Ocurrencia de eventos particulares
-   Precios de otras comodities, etc.
-   Empreses que est·n relacionadas.

```{r}
acciones$Fecha_dup <- acciones$Fecha
acciones <- acciones %>% separate(Fecha, c("AÒo", "Mes", "DÌa"))
```

```{r}
acciones$AÒo <- as.numeric(acciones$AÒo)
acciones$Mes <- as.numeric(acciones$Mes)
acciones$DÌa <- as.numeric(acciones$DÌa)
str(acciones)
```
...
-   `train_data`: es una subconjunto de √≠ndices del DataFrame `acciones` que van a servir para entrenar al modelo.
-   `test_data`: es un subconjunto de `acciones` definido por los otros √≠ndices que no son de entrenamiento y los datos a predecir.
...
Las redes neuronales requieren escalado. Son m√°s lentos, hay modelos que son muchos m√°s r√°pidos. Es muy sensible a las variables.

```{r}
mod = neuralnet(formula = Precios ~ AÒo + Mes + DÌa, data = acciones.sc[train_data,], hidden = 2, threshold = 0.01, stepmax = 1e+08, rep = 1, linear.output = TRUE)
```

AquÌ en este paso calculamos el modelo con los par√°metros que les hemos puesto.
...
```{r}
mod_rf = randomForest(Precio ~ AÒo + Mes + DÌa, data = acciones[train_data,], type = "regression", ntree = 100)
